...
Pooled QC samples within injection sequences are very important to control the quality of untargeted analysis. I added some function in `enviGCMS` package to visualize or analysis Pooled QC samples.
...
For LC/GC-MS analysis, the run order will affect the intensity of single peak until the instrument is stable. The intensity will decrease/increase with initial run order and researcher need to evaluate how many samples are enough to eliminated run order effects. Here I introduced a pooled QC linear index to show such trends in the sequence. Such index would be a value between 0 and 1. The higher of such index, more peaks within that sample would be affected by run order effect. You could use such function to check the very first samples at the beginning of your sequence to see if run order effects would influence the samples in the beginning of sequences.
