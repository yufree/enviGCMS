...
- `getupload` could get the csv files to be submitted to Metaboanalyst from `xcmsSet`/`XCMSnExp`/`list` object

- `getmzrt` could get a list or csv files with peaks list, mz, retention time and class of samples from `xcmsSet`/`XCMSnExp` object
