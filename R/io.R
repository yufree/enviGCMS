...
combinemd <- function(data1, data2, ...) {
    if (missing(...)) {
        z1 <- getmd(data1)
        z2 <- getmd(data2)
        ind <- intersect(round(as.numeric(colnames(z1)), 
            1), round(as.numeric(colnames(z2)), 1))
        ind0 <- intersect(colnames(z1), colnames(z2))
        z <- rbind(z1[, as.character(ind0)], z2[, as.character(ind0)])
        rownames(z) <- c(seq(min(rownames(z1)), max(rownames(z1))), 
            seq(min(rownames(z2)), max(rownames(z2))))
        colnames(z) <- ind
        return(z)
    } else {
