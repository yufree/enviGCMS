...
comparems <- function(data1, data2, lab1 = "", lab2 = "") {
    ...
    graphics::plot(rowSums(data1), type = "l", xlab = "retention time", 
        ylab = "intensity", xaxt = "n", frame.plot = F, 
        main = lab1)
    ind = as.numeric(rownames(data1))
    graphics::axis(1, at = seq(0, 1500, 300), labels = round((ind[1] + 
        (max(ind) - min(ind))/5 * (0:5))/60, 2))
    graphics::plot(rowSums(data2), type = "l", xlab = "retention time", 
        ylab = "intensity", xaxt = "n", frame.plot = F, 
        main = lab2)
    ind = as.numeric(rownames(data2))
    graphics::axis(1, at = seq(0, 1500, 300), labels = round((ind[1] + 
        (max(ind) - min(ind))/5 * (0:5))/60, 2))
    graphics::plot(rowSums(z), type = "l", xlab = "retention time", 
        ylab = "intensity", xaxt = "n", frame.plot = F, 
        main = "New TIC")
...
plotgroup <- function(data, threshold = 2) {
    ...
    graphics::hist(log10(data), breaks = 100, main = "", 
        xlab = "Intensity")
    graphics::abline(v = threshold, lwd = 5, lty = 2, 
        col = "red")
    ...
    graphics::axis(1, at = c(0, rtx[indrt%%300 == 0], 
...
plothist <- function(data) {
    ...
    graphics::legend("topright", c("noise", "signal", 
        "density"), box.lty = 0, pch = c(-1, -1, -1), 
        lty = c(1, 1, 2), lwd = c(2, 2, 2), col = c("red", 
            "green", "black"))
}
