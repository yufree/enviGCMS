...
svaplot <- function(list,
        ...
        plotchange <- function(zlim){
                ...
                p <- par('usr')
                text(p[2]+2, mean(p[3:4]), labels = 'intensity', xpd = NA, srt = -90)
                bks <-
...
svabatch <- function(df, dfsv, dfanova) {
        ...
        p0[is.infinite(p0)] <- max(p0[!is.infinite(p0)])+1

        p1 <- -log10(df$`p-values`)
        p1[is.infinite(p1)] <- max(p1[!is.infinite(p1)])+1

        big <- p0-p1

        plot(
                df$PosteriorProbabilitiesSurrogate[big<0] ~ df$PosteriorProbabilitiesMod[big<0],
                xlab = 'Influences  from primary variable',
                ylab = 'Influences  from surrogate variable',
                main = 'Peaks with larger p-values',
                pch = 19,
                cex = 1.2,
                ylim = c(0, 1),
                xlim = c(0, 1.02),
        ...
        points(df$PosteriorProbabilitiesSurrogate[big<0&dfsv$pqvalues&dfanova$pqvalues] ~ df$PosteriorProbabilitiesMod[big<0&dfsv$pqvalues&dfanova$pqvalue],
               pch = 19, col = 'red')
        points(df$PosteriorProbabilitiesSurrogate[big<0&!dfsv$pqvalues&dfanova$pqvalues] ~ df$PosteriorProbabilitiesMod[big<0&!dfsv$pqvalues&dfanova$pqvalue],
               pch = 19, col = 'green')
        plot(
                ...
                xlab = 'Influences  from primary variable',
                ylab = 'Influences  from surrogate variable',
                main = 'Peaks with smaller p-values',
                pch = 19,
                cex = 1.2,
                ylim = c(0, 1),
                xlim = c(0, 1.02),
