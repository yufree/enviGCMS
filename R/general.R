...
Mode = function(x) {
    ...
    tam = max(ta)
    if (all(ta == tam)) 
        mod = x else if (is.numeric(x)) 
        mod = as.numeric(names(ta)[ta == tam]) else mod = names(ta)[ta == tam]
    return(mod)
}
...
#' }
#' @export
getmd <- function(data, mzstep = 0.1, mzrange = F, rtrange = F) {
...
#' }
#' @export
cbmd <- function(data1, data2, mzstep = 0.1, rtstep = 0.01) {
    ...
    colnames(z) <- intersect(round(as.numeric(colnames(z1)), 
        roundrt), round(as.numeric(colnames(z2)), roundrt))
    return(z)
}
...
#' }
#' @export
submd <- function(data1, data2, mzstep = 0.1, rtstep = 0.01) {
    z1 <- getmd(data1, mzstep = mzstep)
    z2 <- getmd(data2, mzstep = mzstep)
    ...
    rownames(z0) <- rownames(z) <- intersect(round(as.numeric(rownames(z1)), 
        roundmz), round(as.numeric(rownames(z2)), roundmz))
    colnames(z0) <- colnames(z) <- intersect(round(as.numeric(colnames(z1)), 
        roundrt), round(as.numeric(colnames(z2)), roundrt))
    ...
    li <- list(data1 = z10, data2 = z20, data1s2 = z, data2s1 = z0)
    return(li)
}
...
plotms <- function(data, log = F) {
    ...
    graphics::plot(1, 1, t = "n", ylim = c(0, 1), xlim = zlim, 
        xaxt = "n", yaxt = "n", xaxs = "i", yaxs = "i", 
        ylab = "", xlab = "")
    graphics::mtext("intensity", side = 2, line = 0.5, 
        las = 1, cex = 1)
    if (log) {
        graphics::axis(1, at = breaks, labels = round(10^(breaks)), 
            las = 1)
    } else {
        graphics::axis(1, at = breaks, labels = round(breaks), 
            las = 1)
    }
    ...
    bks <- seq(zlim[1], zlim[2], length.out = (length(col) + 
        1))
    for (i in seq(poly)) {
        graphics::polygon(c(bks[i], bks[i + 1], bks[i + 
            1], bks[i]), c(0, 0, 1, 1), col = col[i], border = NA)
    }
    # show the heatmap
    graphics::par(mar = c(4, 5, 0, 4), fig = c(0, 1, 0, 
        0.9), new = T)
    graphics::image(z, ylab = "", axes = F, col = col, 
        useRaster = T)
    ...
    graphics::text(graphics::par("usr")[2] * 1.11, mean(graphics::par("usr")[3:4]), 
        "m/z", srt = -90, xpd = TRUE, pos = 4)
    graphics::text(graphics::par("usr")[2] * 1.05, mzy[indmz%%100 == 
        0][-c(1, sum(indmz%%100 == 0))], labels = indmz[indmz%%100 == 
        0][-c(1, sum(indmz%%100 == 0))], srt = 270, xpd = TRUE)
    graphics::par(mar = c(4, 5, 4, 4), fig = c(0, 1, 0, 
        0.9), new = T, cex.lab = 1)
    data[is.na(data)] <- 0
    data <- apply(data, 2, sum)
    graphics::plot(data ~ indrt, type = "l", ylab = "intensity", 
        xlab = "retention time(s)", frame.plot = F, xaxs = "i", 
        yaxs = "i")
}
...
plotmz <- function(data, inscf = 5, ...) {
    ...
    cexlab = c(paste0(inscf, "-", inscf + 0.5), paste0(inscf + 
        0.5, "-", inscf + 1), paste0(inscf + 1, "-", inscf + 
        1.5), paste0(inscf + 1.5, "-", inscf + 2), paste0(">", 
        inscf + 2))
    ...
    graphics::plot(mz0 ~ rt0, pch = 19, cex = cex, col = grDevices::rgb(0, 
        0, 1, 0.1), xlab = "retention time(s)", ylab = "m/z", 
        ...)
    graphics::legend("top", legend = cexlab, title = "Intensity in Log scale", 
        pt.cex = c(1, 2, 3, 4, 5)/2, pch = 19, bty = "n", 
        horiz = T, cex = 0.7, col = grDevices::rgb(0, 0, 
            1, 0.1), inset = c(0, -0.25))
}
...
#' }
#' @export
plott <- function(data, log = F, temp = c(100, 320)) {
    indmz <- as.numeric(rownames(data))
    indrt <- as.numeric(colnames(data))
    col <- (grDevices::colorRampPalette(rev(RColorBrewer::brewer.pal(11, 
        "RdYlBu"))))(100)
    ...
    graphics::image(z, xlab = expression("Temperature (" * 
        degree * C * ")"), ylab = "m/z", axes = F, col = col, 
        useRaster = T)
    ...
    # display the m/z as y
    mzy <- seq(0, 1, length.out = length(indmz))
    graphics::axis(2, at = mzy[indmz%%100 == 0], labels = indmz[indmz%%100 == 
        0], las = 2)
}
...
plotrtms <- function(data, rt, ms) {
    ...
    graphics::plot(temp, xaxs = "i", yaxs = "i", type = "h", 
        ylab = "intensity", xlab = "m/z", xaxt = "n", frame.plot = F)
...
plotmsrt <- function(data, ms, rt, n = F) {
    ...
    graphics::plot(data ~ x, type = "l", xaxs = "i", yaxs = "i", 
        main = bquote("m/z = " ~ .(ms)), ylab = "intensity", 
        xlab = "retention time(s)", frame.plot = F)
...
plottic <- function(data, n = F) {
    ...
    x <- as.numeric(names(data))
    graphics::plot(data ~ x, type = "l", ylab = "intensity", 
        xlab = "retention time(s)", frame.plot = F)
}
...
plotint <- function(list, name = NULL) {
    ...
    graphics::lines(c(rtstart, rtstart), c(0.8 * sigstart, 
        sigstart * 1.2), "l", col = "blue")
    ...
    graphics::text(rtend, sigpeak * 0.3, as.character(round(rtend, 
        3)), col = "darkgreen")
    graphics::text(rtpeak - 0.1, 0.9 * sigpeak, paste("RT:", 
        as.character(round(rtpeak, 3))), col = "darkgreen")
    graphics::text(rtpeak + 0.1, sigpeak * 0.7, paste("area:", 
        format(area, digits = 2)), col = "red")
    graphics::text(rtpeak + 0.1, sigpeak * 0.9, paste("height:", 
        format(sigpeak, digits = 2)), col = "red")
}
...
plotintslope <- function(list, name = NULL) {
    ...
    sigpeakbase <- peakdata[11]
    lengthsig <- peakdata[12]
    graphics::plot(RTrange, slopedata, xlab = "time (min)", 
        ylab = "slope", "l", main = paste(name, "Slope"))
...
#' }
#' @export
findline <- function(data, threshold = 2, temp = c(100, 
    320)) {
    ...
    graphics::plot(data$y ~ data$x, xlab = expression("Temperature (" * 
        degree * C * ")"), ylab = "m/z", pch = 19, xlim = rangert, 
        ylim = rangemz, xaxt = "n", yaxt = "n", main = "", 
        frame.plot = F)
    # display the temperature as x
    ...
    graphics::axis(2, at = mzy[y0%%100 == 0], labels = y0[y0%%100 == 
        0], las = 2)
    ...
    graphics::text(posx, posy, adj = c(0, 0), cex = 1, 
        eqn)
    graphics::text(posx, posy - (rangemz[2] - rangemz[1]) * 
        0.05, adj = c(0, 0), cex = 1, eqn2)
    graphics::legend("topright", c("OLS", "lowess"), box.lty = 0, 
...
plotgroup <- function(data, threshold = 2) {
    ...
    graphics::layout(m)
    graphics::hist(log10(data), breaks = 100, main = "", 
        xlab = "Intensity")
    graphics::abline(v = threshold, lwd = 5, lty = 2, col = "red")
    graphics::legend("topright", "threshold", box.lty = 0, 
        pch = -1, lty = 2, lwd = 2, col = "red")
    
    graphics::image(t(group), xlab = "retention time(min)", 
        ylab = "m/z", axes = F, col = grDevices::heat.colors(2), 
        useRaster = T)
    indmz <- as.numeric(rownames(data))
    indrt <- as.numeric(colnames(data))
    # display the RT as x
    rtx <- seq(0, 1, length.out = length(indrt))
    graphics::axis(1, at = c(0, rtx[indrt%%300 == 0], 1), 
...
plotsms <- function(meanmatrix, rsdmatrix) {
    graphics::par(mar = c(4.2, 4.2, 0, 1.5), fig = c(0, 
        1, 0, 0.8), new = F, cex.axis = 1.5, cex.lab = 1.5)
    graphics::smoothScatter(y = rsdmatrix * 100, x = log10(c(meanmatrix)), 
        main = "", xlab = "Intensity", ylab = "Relative Standard Deviation(%)", 
        xaxt = "n", frame.plot = F)
    graphics::abline(h = 20, lty = 2, col = "red")
    graphics::abline(h = 10, col = "red")
    graphics::axis(1, at = c(0, 1, 2, 3, 4, 5, 6, 7), labels = c("1", 
        "10", "100", "1000", "10000", "100000", "1000000", 
        "100000000"))
    graphics::par(mar = c(0, 4.2, 1, 1.5), oma = c(0, 0, 
        0, 0), fig = c(0, 1, 0.8, 1), new = T, cex.axis = 1.5, 
        cex.lab = 1.5)
...
plothist <- function(data) {
    ...
    graphics::lines(stats::density(log10(data1)), lty = 2, 
        lwd = 2)
    graphics::legend("topright", c("noise", "signal", "density"), 
        box.lty = 0, pch = c(-1, -1, -1), lty = c(1, 1, 
            2), lwd = c(2, 2, 2), col = c("red", "green", 
            "black"))
}
