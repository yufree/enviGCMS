...
getfeaturest <- function(xod, power = 0.8, pt = 0.05, qt = 0.05, n = 3, rsdt = 50){
        ...
        for (i in c(1:nrow(df))){
                ...
                r <- stats::power.t.test(delta = df$dm[i], sd = df$sd[i], sig.level = df$alpha[i], n = n)
                rp[i] <- r$power
        }
...
getfeaturesanova <- function(xod, power = 0.8, pt = 0.05, qt = 0.05, n = 3, ng = 3, rsdt = 50){
        data <- xcms::groupval(xod, value = "into")
        idx <- complete.cases(data)
        data1 <- as.data.frame(xcms::groups(xod))
        lv <- xod@phenoData[,1]
