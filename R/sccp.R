...
getareastd <- function(data = NULL,ismz = 323, ppm = 5, con = 2000,rt = NULL, rts = NULL){
        ...
        ccomp <- stats::aggregate(rrares,by = list(sccp$Cn),sum)
        colnames(ccomp) <- c("nC","Formula group abundance")
        clcomp <- stats::aggregate(rrares,by = list(sccp$Cln),sum)
...
getsccp <- function(pathstds, pathsample, ismz = 323, ppm = 5, con = 2000, rt = NULL, rts = NULL, log = T){
        ...
        pathsamp <- list.files(path = pathsample, full.names = T, recursive = T)
        nstd <- length(pathstd)
        ...
        for(i in 1:nstd){
                ...
                liststd[[i]] <- getareastd(file,ismz = ismz,ppm = ppm, con = con, rt = rt, rts = rts)
        }
        ...
        for(i in 1:nsamp){
                ...
                listsamp[[i]] <- getarea(file,ismz = ismz,ppm =ppm, rt = rt, rts = rts)
        }
        ...
        return(list(cons = con, Ccomp = ccomp, Clcomp = clcomp, Ccomps = ccomps, Clcomps = clcomps, lmfit = lmfit))
}
